...
- **🏆 全世界ランキング**: ゲームオーバー時にニックネームでスコアを登録し、全世界のプレイヤーと競争(全期間/24時間の2タブ)。Supabase を使い、未設定なら自動で端末内ランキングにフォールバック。設定手順は **[SUPABASE_SETUP.md](./SUPABASE_SETUP.md)**
...
js/
  main.js           起動・入力・DOM 配線・ショップ・ランキング・メインループ
  ...
  leaderboard.js    全世界ランキング(Supabase REST / 端末内フォールバック)
  config.js         Supabase の URL / anon キー設定(ランキング用)
  util.js           汎用ヘルパー
